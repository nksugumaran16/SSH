...
cp swagger.yaml $version/

openapi-generator-cli generate -g spring -i $version/swagger.yaml -c config.json -o spring-boot-codegenerator
...
curl -vSf -u "admin:password" -X PUT -T "target/spring-boot-codegenerator-$version.jar" "http://localhost:8082/artifactory/artifactsjar-generic-local//spring-boot-codegenerator-$version.jar"
